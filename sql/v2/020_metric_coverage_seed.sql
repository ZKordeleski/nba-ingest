...
INSERT INTO metric_coverage (metric, column_ref, first_tracked_season, status, null_means, authority) VALUES
    ...
    -- is NOT an era gap and NOT (by default) an ingestion gap — it most often means the
    -- player DID NOT PLAY (DNP/inactive). Cross-check minutes_played (NULL/0) +
    -- game_inactives to confirm; a NULL alongside minutes_played>0 is instead a real gap
    -- (anomaly worth flagging). Never 0 for not-recorded.
    ('pts',  'player_box_basic.pts / games.*_pts',     NULL, 'always',         'player-game NULL could mean the player did not play (DNP/inactive) — cross-check minutes_played (NULL/0) + game_inactives to confirm; a NULL with minutes_played>0 is instead a real gap (anomaly). Never 0 for not-recorded.', 'tracked since 1946-47'),
    ('fg',   'player_box_basic.fgm/fga',               NULL, 'always',         'player-game NULL could mean the player did not play (DNP/inactive) — confirm via minutes_played + game_inactives; a NULL with minutes_played>0 is instead a real gap.', 'tracked since 1946-47'),
    ('ft',   'player_box_basic.ftm/fta',               NULL, 'always',         'player-game NULL could mean the player did not play (DNP/inactive) — confirm via minutes_played + game_inactives; a NULL with minutes_played>0 is instead a real gap.', 'tracked since 1946-47'),
    ('trb',  'player_box_basic.reb',                   NULL, 'always',         'Pre-1950-51: total rebounds not recorded (era). From 1950-51: a player-game NULL could mean the player did not play (DNP/inactive) — confirm via minutes_played + game_inactives.', 'total rebounds since 1950-51; treat pre-1951 as not recorded'),
    ('pf',   'player_box_basic.pf',                    NULL, 'always',         'player-game NULL could mean the player did not play (DNP/inactive) — confirm via minutes_played + game_inactives; a NULL with minutes_played>0 is instead a real gap.', 'tracked since 1946-47'),
