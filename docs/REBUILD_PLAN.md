...
### Post-validation audit + analyst-query pass (2026-06-18)
Ran the full DB anomaly audit + an analyst-grade query battery (scoring/efficiency/team-record/playoff leaders) corroborated against NBA.com/Wikipedia/ESPN. **Every correctly-written query matched external sources, several to the decimal** (Warriors 73-9, Curry 402 3PM, LeBron 8289 playoff pts, SGA 51.9/37.5/89.8). Two modern-data defects found + fixed (both detailed in BR_DATA_CATALOG):
- **2025 DNP zero-coercion** (NULL!=0 violated by the Phase-2 pilot flattener) → re-loaded 2025 with the current flattener; DNP now NULL. Finding `dnp_zero_coercion:2025` resolved.
- **NBA Cup Championship tagged Regular Season** (2024+2025) → re-tagged `season_type='NBA Cup Championship'`; restores exact official values. Finding resolved. Durable auto-tagging is a backlog item.

Blast radius lesson: the DNP coercion corrupted ONLY naive `COUNT(*)`-as-games-played / `AVG(rate)` queries in 2025 — aggregate-then-divide (the correct form) was immune. Neither defect corrupted sound analysis; both are now fixed regardless. The audit's other 17 flags are documented era-ramps / signed caveats / detector-logic gaps (the latter tracked: suppress collision-caveated advanced dups; apply ramp logic to the quarters-reconcile detector).
...
| **NBA Cup Championship auto-tagging in ingest** | game · scorebox "NBA Cup" label + latest-date | The Cup Championship is the ONLY tournament game excluded from reg-season stats/standings (group+knockout+semis all count). The index-driven enumerator now CAPTURES it, but the loader tagged it `Regular Season` → polluted ~51 finalist lines across 2024+2025 (RS count 1231 vs 1230). **Data fixed 2026-06-18** by re-tagging `season_type='NBA Cup Championship'` on `202312090LAL`+`202412170OKC` (+ their player_box). REMAINING: make it durable — the loader must auto-detect & tag. Detection design: BR scorebox_meta labels Cup games `NBA Cup`, but **semifinals share the identical label** (also neutral-site Las Vegas), so the championship = the **latest `NBA Cup`-labeled game per season** (≥2024). Cleanest impl: capture the BR label in `br_client._parse_meta` → carry through flatten (e.g. `round='NBA Cup'`) → loader post-step tags the season's latest such game `NBA Cup Championship`. Avoid hardcoding "Las Vegas" (venue could move). Needs re-load of 2024+2025 to populate the label. | annual Cup season ingest / cutover daily-path | **Data fixed; auto-tagging pending** |
