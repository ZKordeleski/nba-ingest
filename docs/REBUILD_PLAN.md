...
- **Caveat honesty** — (a) dimension-scoped suppression: a caveat silences *only its own* detector, so a caveated game still runs every other check ("caveated ∴ all-clear" is a category error); (b) a caveat-rate meta-detector: proliferation of one caveat type = a systematic bug wearing a per-game costume (baseline ~0.14% = 9/6243; a season >2–3% flags); (c) **symmetric ceilings** (2026-06-10, Zack's call — "feels risky to not quarantine anything flagged, even with caveats"): every caveat type has an upper bound above which it is **bug-sized → quarantine for human review**, never silently admitted. Reconciliation already had `CAVEAT_RECON_MAX=6`; the asymmetry was that `line_score_discrepancy` was **unbounded** — and the risk was *already realized* (game `195003160DNN` admitted with a magnitude-**57** line-score caveat: a 1950 BAA box score where only ~half the quarter cells parsed; final score correct, quarter split bogus). Added `CAVEAT_LINESCORE_MAX=6` (splits the two physical comparisons: quarters-vs-total incompleteness and line-total-vs-game-total disagreement) and `CAVEAT_COLLISION_MAX=4` (2–3 same-name players = caveat; more = slug-resolution bug). Quarantine (not surgical NULL) chosen so the game lands in the rich worklist with magnitudes attached and a human re-admits it responsibly. Blast radius: 9 already-loaded games (8× 1949–50, 1× 1963) → remediated at the gate; 11 games / 13 sub-ceiling caveats stay admitted.
...
3. `_remediate_linescore.py` (preview, then `--apply`) — quarantine the 9 over-ceiling games for review.
4. `_audit.py --completeness` across all loaded eras — the first run that tests *absence* for real.
5. (loader swap is already on `main`; the next chunk picks up `insert_quarantine` + drain automatically.)
